...
function createProduct(req, res, next) {
	...
	const { name, description, info, imageUrl, price, quantity, category } = req.body;
	productModel.create({ name, description, info, imageUrl, price, quantity, category, userId })
		.then(product => {
			return userModel.updateOne({ _id: userId }, { $push: { products: product._id } });
		}).then(product => {
			console.log(product);
